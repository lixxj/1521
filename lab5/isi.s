...
############ body of isIdent(m, N) ####################################
	li 		$t0, 0 # matrix offset
	li 		$t1, 0 # row index
	li 		$t2, 0 # column index
	lw   	$t3, N # matrix dimension
	li 		$v0, 1 # default return true
	
row_loop:
	beq		$t1, $t3, end_row_loop
	li   	$t2, 0 # reset column index

column_loop:
	beq		$t2, $t3, end_column_loop
	lw 		$t4, m($t0) # value of current position
	beq		$t1, $t2, main_diagonal # elements of the main diagonal

not_main_diagonal:
	bnez	$t4, set_false
	j 		column_loop_continue

main_diagonal:
	li 		$t5, 1
	beq 	$t5, $t4, column_loop_continue

set_false:
	li 		$v0, 0 # return false

column_loop_continue:
	addi	$t2, $t2, 1 # increment column index
	addi	$t0, $t0, 4 # increment matrix offset
	j 		column_loop
